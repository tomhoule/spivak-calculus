...
assume anotzero h,
calc
    b   = a⁻¹ * a * b : by simp [one_mul, inv_mul_cancel anotzero]
...
calc
    a^2 - b^2   = a * (a + b - b) - (b * (b + a - a)) : by simp [pow_two]
            ... = a * (a + b) + a * -b - (b * (b + a) + b * -a) : by simp [mul_add]
            ... = a * (a + b) + a * -b + -(b * (b + a) + b * -a) : rfl
            ... = a * (a + b) + a * -b + -(b * (b + a)) + -(b * -a) : by simp [neg_add, add_assoc]
            ... = (a + b) * a + a * -b + (a + b) * -b + b * a : by simp [add_comm, mul_comm]
...
calc
    a^3 - b^3   = a * a^2 - b * b^2 : by simp [pow_succ]
            ... = a * ((a^2 - b^2) + b^2) - b * b^2 : by simp
            ...
            ... = (a - b) * (a^2 + a * b + b^2) : by simp [mul_add, pow_succ]
...
section v
    ...
    have l1 : a * expanded_n = middle, from (
        ...
        a * expanded_n  = ∑ m in finset.range (n+1), a * (a^(n - m) * b^m) : by rw finset.sum_hom (finset.range (n+1)) (λ x, a * x)
                    ... = ∑ m in finset.range (n+1), (a * a^(n - m)) * b^m : by simp [mul_assoc]
    ...
    | (n+2) h :=
        ...
        a^(n+2) - b^(n+2)   = a * a^(n+1) - (b * b^(n+1)) : by rw [pow_succ a, pow_succ b]
                        ...
                        ... = (a - b) * (a * gizmo a b n) + (a - b) * b^(n+1) : by simp [mul_comm a, mul_assoc]
